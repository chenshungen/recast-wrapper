...
void dtCrowd::resetAgent(int idx, const float* pos, const dtCrowdAgentParams* params)
{
	if (idx <= 0)
		return;
...
void dtCrowd::update(const float dt, dtCrowdAgentDebugInfo* debug)
{
	...
	for (int i = 0; i < nagents; ++i)
	{
		...
		else
		{
			...
			const float slowDownRadius = 0.1f;
			float speedScale = 1.0f;
			float distanceToGoal = getDistanceToGoal(ag, slowDownRadius);
			if (distanceToGoal < slowDownRadius) 
				speedScale = 0;
			else
